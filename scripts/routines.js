...
var reparations  = cron.schedule( period, async () => {
    let today = new Date()
...
var delay = cron.schedule(period, async() =>{
    let today = new Date()
    ...
    for (const rent of rents) {
        for (const item of rent.products) {
            await support.replaceItemPeriod(item, support.addDays(today, 1), support.addDays(today, days))
        }
        if(rent.state != 'delayed')
            console.log(await Rent.updateOne({ _id: rent._id }, { state: 'delayed' }))
    }
}, { scheduled: false })
